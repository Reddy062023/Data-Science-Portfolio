...
Author: Japendra
Projects: Sales EDA, Healthcare Analytics, Finance ML, SQL Analytics
"""
...
@st.cache_data
def load_sql_files():
    sql_dir = ROOT_DIR / "SQL-Analytics" / "queries"
    files = {
        "01 - Basic Queries":        "01_basic_queries.sql",
        "02 - JOINs":                "02_joins.sql",
        "03 - Aggregations":         "03_aggregations.sql",
        "04 - Window Functions":     "04_window_functions.sql",
        "05 - Subqueries & CTEs":    "05_subqueries_ctes.sql",
        "06 - Stored Procedures":    "06_stored_procedures.sql",
    }
    content = {}
    for label, filename in files.items():
        ...
        if path.exists():
            ...
            content[label] = f"-- File not found: {filename}"
    return content
...
st.sidebar.markdown("Data Scientist | Python | ML | SQL")
st.sidebar.markdown("---")

page = st.sidebar.radio(
    "Navigate",
    ["🏠 Home", "📊 Sales Analytics", "🏥 Healthcare Analytics", "💰 Finance ML", "🗄️ SQL Analytics"]
)
...
st.sidebar.markdown(f"🗄️ SQL: 12 tables · 50+ queries")
...
if page == "🏠 Home":
    ...
    st.markdown("### End-to-end data analysis, visualization, machine learning and SQL projects")
    st.markdown("---")

    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Total Projects", "4")
    ...
    with col2:
        ...
        st.metric("Readmission Rate", f"{health['readmitted_30d'].mean()*100:.1f}%")
    ...
    with col3:
        ...
        st.metric("Default Rate", f"{finance['defaulted'].mean()*100:.1f}%")

    with col4:
        st.markdown("### 🗄️ SQL Analytics")
        st.markdown("**12 tables · 4 domains**")
        st.markdown("- E-Commerce queries")
        st.markdown("- Healthcare queries")
        st.markdown("- Finance queries")
        st.markdown("- HR & Window Functions")
        st.metric("SQL Queries", "50+")
...
# ----------------------------------------------------------------
    ...
    col4.metric("Return Rate",     f"{pd.to_numeric(filtered['returned'], errors='coerce').mean()*100:.1f}%")
    ...
    with col2:
        ...
        fig = px.bar(channel_rev, x="channel", y="revenue", template="plotly_white",
...
# ----------------------------------------------------------------
    ...
    with col1:
        ...
        fig = px.pie(diag_counts, values="count", names="diagnosis",
                     color="diagnosis",
                     color_discrete_map={"Healthy":"#2ecc71","At Risk":"#f39c12","Critical":"#e74c3c"},
                     template="plotly_white")
        fig.update_traces(textinfo="percent+label")
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        ...
        fig = px.bar(readmit, x="readmission_rate", y="insurance_type", orientation="h",
                     template="plotly_white", color="readmission_rate",
                     color_continuous_scale=["#2ecc71","#e74c3c"],
                     labels={"readmission_rate":"Readmission Rate (%)","insurance_type":"Insurance Type"})
        fig.update_layout(yaxis={"categoryorder":"total ascending"})
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col1:
        ...
        fig = px.scatter(filtered, x="bmi", y="blood_pressure_systolic", color="diagnosis",
                         color_discrete_map={"Healthy":"#2ecc71","At Risk":"#f39c12","Critical":"#e74c3c"},
                         opacity=0.5, template="plotly_white",
                         labels={"bmi":"BMI","blood_pressure_systolic":"Systolic BP"})
        st.plotly_chart(fig, use_container_width=True)

    with col2:
        st.markdown("#### Hospital Days by Diagnosis")
        fig = px.box(filtered, x="diagnosis", y="hospital_days", color="diagnosis",
                     color_discrete_map={"Healthy":"#2ecc71","At Risk":"#f39c12","Critical":"#e74c3c"},
                     template="plotly_white",
                     labels={"diagnosis":"Diagnosis","hospital_days":"Hospital Days"})
        st.plotly_chart(fig, use_container_width=True)

    st.markdown("#### Clinical Metrics Summary")
    clinical_cols = ["age","bmi","blood_pressure_systolic","blood_pressure_diastolic",
...
# ----------------------------------------------------------------
    ...
    with tab1:
        ...
        with col1:
            ...
            fig = px.bar(grade_default, x="loan_grade", y="default_rate",
                         template="plotly_white", color="default_rate",
                         color_continuous_scale=["#2ecc71","#e74c3c"],
        ...
        with col2:
            ...
            fig = px.histogram(filtered, x="credit_score",
                               color=filtered["defaulted"].map({0:"No Default",1:"Default"}),
                               barmode="overlay", opacity=0.6, template="plotly_white",
                               color_discrete_map={"No Default":"#2ecc71","Default":"#e74c3c"},
                               labels={"credit_score":"Credit Score","color":"Status"})
            st.plotly_chart(fig, use_container_width=True)
        ...
        with col1:
            ...
            fig = px.bar(purpose_default, x="default_rate", y="loan_purpose", orientation="h",
                         template="plotly_white", color="default_rate",
                         color_continuous_scale=["#2ecc71","#e74c3c"],
        ...
        with col2:
            ...
            if "debt_to_income" in filtered.columns:
                fig = px.scatter(filtered, x="debt_to_income", y="credit_score",
                                 ...
                                 color_discrete_map={"No Default":"#2ecc71","Default":"#e74c3c"},
                                 labels={"debt_to_income":"Debt-to-Income","credit_score":"Credit Score","color":"Status"})
                st.plotly_chart(fig, use_container_width=True)
    ...
    with tab2:
        ...
        with col1:
            ...
            income_annual = st.number_input("Annual Income ($)", 10000, 300000, 60000, step=1000)
        ...
        with col3:
            employment_status = st.selectbox("Employment Status",
                                             sorted(finance["employment_status"].dropna().unique().tolist()))
            loan_purpose      = st.selectbox("Loan Purpose",
                                             sorted(finance["loan_purpose"].dropna().unique().tolist()))
            loan_grade        = st.selectbox("Loan Grade",
                                             sorted(finance["loan_grade"].dropna().unique().tolist()))
        ...
        if st.button("🔮 Predict Default Risk", type="primary"):
            ...
            if prediction == 1:
                col1.error("⚠️ HIGH RISK - Likely to Default")
            else:
                col1.success("✅ LOW RISK - Unlikely to Default")
...
# ----------------------------------------------------------------
    ...
    st.title("🗄️ SQL Analytics - Multi-Domain Database Project")
    st.markdown("---")

    # Stats
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Domains", "4")
    col2.metric("Tables", "12")
    col3.metric("SQL Files", "6")
    col4.metric("Total Queries", "50+")

    st.markdown("---")

    # Domain overview
    st.markdown("## Database Domains")
    col1, col2, col3, col4 = st.columns(4)

    with col1:
        st.markdown("### 🛒 E-Commerce")
        st.markdown("- `customers` — 10 rows")
        st.markdown("- `products` — 10 rows")
        st.markdown("- `orders` — 15 rows")
        st.markdown("- `order_items` — 23 rows")

    with col2:
        st.markdown("### 🏥 Healthcare")
        st.markdown("- `patients` — 10 rows")
        st.markdown("- `doctors` — 5 rows")
        st.markdown("- `admissions` — 15 rows")

    with col3:
        st.markdown("### 💰 Finance")
        st.markdown("- `borrowers` — 10 rows")
        st.markdown("- `loans` — 15 rows")

    with col4:
        st.markdown("### 👥 HR")
        st.markdown("- `departments` — 5 rows")
        st.markdown("- `employees` — 15 rows")
        st.markdown("- `salaries` — 10 rows")

    st.markdown("---")

    # SQL Concepts covered
    st.markdown("## SQL Concepts Covered")
    col1, col2, col3 = st.columns(3)

    with col1:
        st.info("**01 - Basic Queries**\nSELECT, WHERE, ORDER BY, LIMIT, DISTINCT — 15 queries")
        st.info("**02 - JOINs**\nINNER, LEFT, RIGHT, SELF JOIN, multi-table — 10 queries")

    with col2:
        st.info("**03 - Aggregations**\nCOUNT, SUM, AVG, GROUP BY, HAVING — 13 queries")
        st.info("**04 - Window Functions**\nRANK, DENSE_RANK, LAG, LEAD, PARTITION BY — 10 queries")

    with col3:
        st.info("**05 - Subqueries & CTEs**\nWITH clauses, nested queries, CASE WHEN — 9 queries")
        st.info("**06 - Stored Procedures**\nIN/OUT params, IF/ELSE, DECLARE — 5 procedures")

    st.markdown("---")

    # SQL Query Browser
    st.markdown("## 📂 Browse SQL Queries")
    selected_file = st.selectbox("Select a query file to view:", list(sql_files.keys()))

    if selected_file:
        st.markdown(f"### {selected_file}")
        st.code(sql_files[selected_file], language="sql")
    ...
    questions = {
        ...
        "🏥 Healthcare": [
            "Which doctors generate most revenue?",
            "Which patients have highest admission costs?",
            "What is avg cost by insurance type?"
        ],
        "💰 Finance": [
            "What is default rate by loan grade?",
            "Which borrowers are highest risk?",
            "What is total exposure by loan purpose?"
        ],
        "👥 HR": [
            "Which departments pay above average?",
            "Who earns above their department average?",
            "What is the salary history per employee?"
        ]
    }
    ...
    for i, (domain, qs) in enumerate(questions.items()):
        with cols[i]:
