...
# ─── Fix missing values before plotting ─────────────────────────
df["bmi"] = df["bmi"].fillna(df["bmi"].median())
df["glucose_level"] = df["glucose_level"].fillna(df["glucose_level"].median())
df["cholesterol_total"] = df["cholesterol_total"].fillna(df["cholesterol_total"].median())

# ─── Save updated/cleaned CSV ───────────────────────────────────
df.to_csv("healthcare_data_clean.csv", index=False)
print("Saved cleaned CSV: healthcare_data_clean.csv")
...
fig.suptitle("Patient Population Overview — Healthcare Dataset", fontsize=15, fontweight="bold", y=1.01)
...
plt.tight_layout()
chart1_path = os.path.join(plots_dir, "chart1_population_overview.png")
...
sns.heatmap(corr, mask=mask, annot=True, fmt=".2f", cmap="RdYlGn",
            ...
            annot_kws={"size": 9})
ax.set_title("Feature Correlation Matrix — Healthcare", fontsize=14, fontweight="bold", pad=20)
plt.tight_layout()
chart2_path = os.path.join(plots_dir, "chart2_correlation.png")
...
df["age_group"] = pd.cut(df["age"], bins=[18,35,50,65,90],
                          labels=["18-35","35-50","50-65","65+"])
crit_rate = df.groupby("age_group", observed=True).apply(
    lambda g: (g["diagnosis"] == "Critical").mean() * 100).reset_index()
...
chart3_path = os.path.join(plots_dir, "chart3_risk_factors.png")
...
print("\n✓ Phase 2 complete. Clean CSV and 3 charts saved in 'plots/' folder.")